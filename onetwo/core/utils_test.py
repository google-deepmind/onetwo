...
# Extra tolerance added to upper-bound timing assertions to prevent flaky
# failures caused by scheduling jitter on shared test infrastructure.
_TIMING_SLACK = 0.05
...
class UtilsTest(parameterized.TestCase):
  ...
  def test_with_retry_sync(self):
    ...
    with self.subTest('should_introduce_no_delay_if_no_retry_is_needed'):
      ...
      self.assertLess(
          end_time - start_time, 0.5 * initial_base_delay + _TIMING_SLACK
      )
    ...
    with self.subTest('first_retry_should_take_around_initial_base_delay'):
      ...
      self.assertLess(
          end_time - start_time, 2.1 * initial_base_delay + _TIMING_SLACK
      )
    ...
    with self.subTest('second_retry_should_double_the_base_delay'):
      ...
      self.assertLess(
          end_time - start_time, 6.1 * initial_base_delay + _TIMING_SLACK
      )
    ...
    with self.subTest('third_retry_should_keep_base_delay_constant'):
      ...
      # Maximum delay is double the minimum delay (due to the randomization).
      self.assertLess(
          end_time - start_time, 10.1 * initial_base_delay + _TIMING_SLACK
      )
    ...
    with self.subTest('base_delay_should_be_capped_at_max_base_delay'):
      ...
      self.assertLessEqual(
          end_time - start_time, 2 * 10 * max_base_delay + _TIMING_SLACK
      )
...
class WithRetryRpcTrackerTest(parameterized.TestCase):

  def setUp(self):
    super().setUp()
    self.tracker = rpc_tracker_lib.RpcTracker()
    # RpcTracker starts a periodic logging thread by default; stop it to
    # avoid noisy log output during tests.
    self.tracker.stop_periodic_logging()
    rpc_tracker_lib.set_rpc_tracker(self.tracker)

  def tearDown(self):
    super().tearDown()
    self.tracker.stop_periodic_logging()
    rpc_tracker_lib._tracker = None

  def test_sync_success_records_start_and_success(self):
    @utils.with_retry(
        max_retries=3, initial_base_delay=0.001, max_base_delay=0.001
    )
    def succeed():
      return 42
    ...
    with self.subTest('returns_correct_value'):
      self.assertEqual(result, 42)
    with self.subTest('one_successful_call'):
      self.assertEqual(stats['successful'], {'succeed': 1})
    with self.subTest('no_errors'):
      self.assertEqual(stats['errors'], {})
    with self.subTest('no_retries'):
      self.assertEqual(stats['total_retries'], 0)
    with self.subTest('nothing_in_flight'):
      self.assertEqual(stats['in_flight_count'], 0)
    with self.subTest('one_completed_duration'):
      self.assertLen(stats['completed_durations'], 1)

  def test_async_success_records_start_and_success(self):
    @utils.with_retry(
        max_retries=3, initial_base_delay=0.001, max_base_delay=0.001
    )
    async def succeed():
  ...
  def test_sync_no_retry_success_records_start_and_success(self):
    @utils.with_retry(
        max_retries=0, initial_base_delay=0.001, max_base_delay=0.001
    )
    def succeed():
      return 42
    ...
    with self.subTest('nothing_in_flight'):
      self.assertEqual(stats['in_flight_count'], 0)
    with self.subTest('total_invocations'):
      self.assertEqual(stats['total_invocations'], 1)

  def test_async_no_retry_success_records_start_and_success(self):
    @utils.with_retry(
        max_retries=0, initial_base_delay=0.001, max_base_delay=0.001
    )
    async def succeed():
  ...
  def test_async_no_retry_failure_records_failure(self):
    @utils.with_retry(
        max_retries=0, initial_base_delay=0.001, max_base_delay=0.001
    )
    async def fail():
    ...
    with self.subTest('no_successful_calls'):
      self.assertEqual(stats['successful'], {})
    with self.subTest('one_error'):
      self.assertEqual(stats['errors'], {'fail': 1})
    with self.subTest('error_type_recorded'):
      self.assertEqual(stats['error_types'], {'ValueError': 1})
    with self.subTest('nothing_in_flight'):
      self.assertEqual(stats['in_flight_count'], 0)

  def test_sync_immediate_failure_records_failure(self):
    @utils.with_retry(
        max_retries=0, initial_base_delay=0.001, max_base_delay=0.001
    )
    def fail():
  ...
  def test_async_immediate_failure_records_failure(self):
    @utils.with_retry(
        max_retries=0, initial_base_delay=0.001, max_base_delay=0.001
    )
    async def fail():
  ...
  def test_sync_retries_then_succeeds(self):
    ...
    with self.subTest('retries_recorded'):
      self.assertEqual(stats['retries'], {'flaky': 2})
      self.assertEqual(stats['total_retries'], 2)
    with self.subTest('one_successful_call'):
      self.assertEqual(stats['successful'], {'flaky': 1})
    with self.subTest('no_final_errors'):
      self.assertEqual(stats['errors'], {})
    with self.subTest('nothing_in_flight'):
      self.assertEqual(stats['in_flight_count'], 0)
  ...
  def test_sync_retries_exhausted_records_failure(self):
    @utils.with_retry(
        max_retries=2, initial_base_delay=0.001, max_base_delay=0.001
    )
    def always_fail():
    ...
    with self.subTest('retries_recorded'):
      self.assertEqual(stats['retries'], {'always_fail': 2})
    with self.subTest('final_error_recorded'):
      self.assertEqual(stats['errors'], {'always_fail': 1})
    with self.subTest('no_successes'):
      self.assertEqual(stats['successful'], {})
    with self.subTest('nothing_in_flight'):
      self.assertEqual(stats['in_flight_count'], 0)

  def test_async_retries_exhausted_records_failure(self):
    @utils.with_retry(
        max_retries=2, initial_base_delay=0.001, max_base_delay=0.001
    )
  ...
  def test_non_retriable_error_records_failure_no_retry(self):
    @utils.with_retry(
        max_retries=3,
        initial_base_delay=0.001,
        max_base_delay=0.001,
        retriable_error_filter=lambda e: isinstance(e, ValueError),
    )
    ...
    with self.subTest('no_retries'):
      self.assertEqual(stats['total_retries'], 0)
    with self.subTest('one_error'):
      self.assertEqual(stats['errors'], {'fail_with_type_error': 1})
    with self.subTest('error_type_recorded'):
      self.assertEqual(stats['error_types'], {'TypeError': 1})

  def test_no_tracker_set_does_not_error(self):
    rpc_tracker_lib._tracker = None

    @utils.with_retry(
        max_retries=1, initial_base_delay=0.001, max_base_delay=0.001
    )
    def succeed():
      return 'ok'
  ...
  def test_multiple_calls_accumulate_stats(self):
    ...
    with self.subTest('two_successes'):
      self.assertEqual(stats['successful'], {'func': 2})
    with self.subTest('one_error'):
      self.assertEqual(stats['errors'], {'func': 1})
    with self.subTest('nothing_in_flight'):
      self.assertEqual(stats['in_flight_count'], 0)
    with self.subTest('three_completed_durations'):
      self.assertLen(stats['completed_durations'], 3)

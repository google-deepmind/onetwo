...
_API_KEY = flags.DEFINE_string('api_key', default=None, help='OpenAI API key.')
...
def main(argv: Sequence[str]) -> None:
  ...
  fname = os.path.join(_CACHE_DIR.value, 'google_openai_api.json')
  ...
  res = executing.run(
      llm.generate_text(prompt='In winter ', max_tokens=5, include_details=True)
  )
  if (res[1]['score'] == 0.0):
    success = False
